...
import { IBaseAsyncThunk, IAssetBaseThunk } from "../slices/interfaces";
import { RedeemHelper } from "../typechain";
import { abi as ierc20Abi } from "../abi/IERC20.json";
import { EnvHelper } from "../helpers/Environment";

export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const defi_busd_address = defi_busd.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(defi_busd_address || "", PairContractABI, provider);
  const reserves = await pairContract.getReserves();
  const marketPrice = (Number(reserves[1].toString())/Math.pow(10,18))/ (Number(reserves[0].toString())/Math.pow(10,9)+1);
  return marketPrice;
}

export async function getAssetAmount({reserveContractAddress, network, decimal} :IAssetBaseThunk) {
  const provider = ethers.getDefaultProvider(network);
  const tokenContract = new ethers.Contract(reserveContractAddress,ierc20Abi, provider );
  const balance = await tokenContract.balanceOf("0x0705FcF83a340ebADe5C771c6ac2e9383851563D")
  console.log("decimals------------------>>>",decimal, typeof(decimal))
  // formatEthers takes BigNumber => String
  let balanceVal = ethers.utils.formatEther(balance);
  console.log("balanceBal--->>", typeof(balanceVal), balanceVal)
  return decimal==9? Number(balanceVal)*Math.pow(10,9):Number(balanceVal) ;
}
 ...
 */
export async function getTokenPrice(tokenId = "defi") {
  // alert("dddddddddddddd")
  let resp;
  try {
    resp = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=${tokenId}&vs_currencies=usd`);
    if(resp.data[tokenId]?.usd){
      return resp.data[tokenId].usd ;
    } else{

      resp = await axios.get('https://www.dextools.io/chain-bsc/api/pancakeswap/poolx?pairSelected=0x80c90bf13709dd9ed72ea6df49ed01b21debbbd7',{
        headers: {'Access-Control-Allow-Origin': '*'}
    })
      console.log("resp-->>",resp)
      const reserve0 = resp.data.pair.reserve0;
      const reserve1 = resp.data.pair.reserve1;
      return (reserve0+1)/reserve1;
    }
...
export function contractForDividEnd({
  networkID,
  provider,
}: {
  networkID: number;
  ...
  return new ethers.Contract(
    addresses[networkID].DIVIDEND_ADDRESS as string,
    DividEndABI,
    provider,
  )} 
