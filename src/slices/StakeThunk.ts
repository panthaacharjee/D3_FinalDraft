...
import { IERC20, OlympusStakingv2 } from "../typechain";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const ohmContract = new ethers.Contract(addresses[networkID].D3_ADDRESS as string, ierc20ABI, signer) as IERC20;
...
export const changeStake = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    const signer = provider.getSigner();
    console.log("signer-->>",networkID,signer);
    ...
    try {
      if (action === "stake") {
        console.log(stakingHelper, ethers.utils.parseUnits(value, "gwei"))
        uaData.type = "stake";
